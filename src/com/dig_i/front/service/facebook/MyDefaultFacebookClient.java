...
public class MyDefaultFacebookClient extends DefaultFacebookClient implements
		MyFacebookClient {
	...
	public String buildLoginUrl(String appId, String redirectUri,
		...
		sb.append(FACEBOOK_OAUTH_ENDPOINT_URL).append("?client_id=")
	...
	public AccessToken obtainAccessToken(String appId, String appSecret,
		...
		try {
			...
			throw new FacebookResponseContentException(
					"Unable to extract access token from response.", t);
		}
	}

	public <T> List<T> search(String type, Class<T> clazz, String query,
			String fields, int limit) throws FacebookException {
		Connection<T> publicSearch = fetchConnection("search", clazz,
				Parameter.with("q", query), Parameter.with("fields", fields),
				Parameter.with("limit", limit), Parameter.with("type", type));
