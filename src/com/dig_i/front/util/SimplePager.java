...
public class SimplePager<T> {
	...
	public SimplePager(int limit, int page, List<T> items, int total,
		...
		this.firstIndice = (this.page - 1) * this.limit + 1;
		this.lastIndice = this.items == null ? this.firstIndice
				: this.firstIndice + this.items.size() - 1;
		...
		if (this.page > Math.ceil(this.totalPage - this.pageRange / 2)) {
			this.startPage = this.totalPage - this.page
					+ (int) (Math.ceil(this.page - (this.pageRange - 1)));
		} else {
			this.startPage = Math.ceil(this.page - (this.pageRange - 1) / 2) > 1 ? (int) Math
					.ceil(this.page - (this.pageRange - 1) / 2) : 1;
		}
		if (this.startPage < 1) {
			this.startPage = 1;
		}
